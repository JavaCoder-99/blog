...
<div class="container" style="width: 90%;">
    ...
    <c:if test="${identity_id ne 4}">
        ...
        <div class="form table">
            <div>
                <form class="layui-form form-inline" action="<%=basePath%>/article/findByCondition" role="form" method="post">
    ...
    <c:if test="${identity_id ne 4}">
        <div class="layui-inline">
            ...
            <a href="<%=basePath%>/article/toArticleWrite"><button type="button" id="addBtn" class="layui-btn layui-btn">添加文章</button></a>
        </div>
    </c:if>
    <br/>
    <br/>
            ...
            <c:forEach items="${page.list}" var="article">
                <tr>
                    ...
                    <c:if test="${identity_id ne 4}">
                        ...
                        <td>
                            <a href="<%=basePath%>/article/toArticleView?r_id=${article.r_id}"><button type="button" class="layui-btn layui-btn-sm">查看</button></a>
                            <a href="<%=basePath%>/article/toEditPage?r_id=${article.r_id}"><button type="button" class="layui-btn layui-btn-sm layui-btn-normal">编辑</button></a>
                            <button type="button" onclick="return clean(${article.r_id});" class="layui-btn layui-btn-sm layui-btn-danger">删除</button>
                        </td>
                    </c:if>
                    <c:if test="${identity_id eq 4}">
                        <td>
                            <a href="<%=basePath%>/article/toArticleView?r_id=${article.r_id}"><button type="button" class="layui-btn layui-btn-sm">查看文章</button></a>
                        </td>
                    </c:if>
                </tr>
            </c:forEach>
    ...
    <form class="listForm" name="listForm" method="post" action="<%=basePath%>/article/findByPage">
        <div class="row">
            <div class="form-inline" style="margin-left: 15px;">
                <label style="font-size:14px;margin-top:22px;">
                    <strong>共<b>${page.total}</b>条记录，共<b>${page.pages}</b>页</strong>
                    &nbsp;
                    &nbsp;
                    <strong>每页显示</strong>
                ...
                <ul class="pagination" style="float:right;">
                    <li>
                        <a href="<%=basePath%>/article/findByPage?pageCode=1"><strong>首页</strong></a>
                    </li>
                    <li>
                        <c:if test="${page.pageNum > 2}">
                            <a href="<%=basePath%>/article/findByPage?pageCode=${page.pageNum-1}">&laquo;</a>
                        </c:if>
                    </li>
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:set var="begin" value="${page.pages - 1}"/>
                            <c:set var="end" value="${page.pages + 3}"/>
                    ...
                    <c:forEach var="i" begin="${begin}" end="${end}">
                        ...
                        <c:if test="${i != page.pageNum}">
                            <li>
                                <a href="<%=basePath%>/article/findByPage?pageCode=${i}">${i}</a>
                            </li>
                        </c:if>
                    </c:forEach>
                    ...
                    <li>
                        <c:if test="${page.pageNum < page.pages}">
                            <a href="<%=basePath%>/article/findByPage?pageCode=${page.pageNum + 1}">&raquo;</a>
                        </c:if>
                    </li>
                    <li>
                        <a href="<%=basePath%>/article/findByPage?pageCode=${page.pages}"><strong>末页</strong></a>
                    </li>
                </ul>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function edit(r_id){
        $.ajax({
            url: '<%=basePath%>/article/toEditPage',
            type: 'GET',
            data: {r_id: r_id},
            success: function(data){
                $("body").html(data);
